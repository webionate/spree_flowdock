...
describe "Checkout" do
  ...
  def create_request_stub(order)
    ...
    stub_request(:post, "https://api.flowdock.com/v1/messages/team_inbox/test")
      .with(
        :body => "source=Spree%20Test%20Store&format=html&from_address=spree%40example.org&subject=New%20incoming%20order%20at%20Spree%20Test%20Store!&content=Order%20number%3A%20#{order.number}%3Cbr%2F%3ETotal%3A%20%24#{order.total}0&from_name=Spree%20Test%20Store&tags=test%2Corder%2Cincoming%2C#{order.number}&link=www.example.org%2Fde%2Fadmin%2Forders%2F#{order.number}%2Fedit",
